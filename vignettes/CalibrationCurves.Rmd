...
In certain scenarios, we have hierarchically structured data. Due to this structure, there will be heterogeneity between clusters and dependency among observations belonging to the same cluster. It is of utmost importance that this is accurately reflected in our predictive mode. A prime example is when patients (level 1) are nested within hospitals (level 2). In these cases, the observations within the same cluster (e.g., hospital) may be more similar to each other than to those belonging to different clusters.

@ClusteredCalibration propose three approaches to obtain calibration curves
