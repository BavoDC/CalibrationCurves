...
# Assessing the calibration of prediction models in clustered datasets
In certain scenarios, we have hierarchically structured data. Here, we have heterogeneity between clusters and dependency among observations belonging to the same cluster. It is of utmost importance that this hierarchical structure is taken into account when assessing the calibration of a predictive model. A prime example of this hierarchical structure is when patients (level 1) are nested within hospitals (level 2). In these cases, the observations within the same cluster may be more similar to each other than to those belonging to different clusters.

We assume that the validation data set has a total of $J$ clusters and use $j = (1, \dots, J)$ to index the clusters. In each cluster, we have $n_j$ patients and we index the patients using $i = (1, \dots, n_j)$. The total sample size is $N = \sum_{j = 1}^J n_j$. We use $y_{ij} \in (0,1)$ to denote the outcome of patient $i$ in cluster $j$, which takes on the value 0 in case of a non-event and 1 in case of an event. We assume that $y_{ij}$ follows a Bernouilli distribution $y_{ij} \sim \text{Bern}(\pi_{ij})$, where $\pi_{ij}$ denotes the probability of experiencing the event. Similar to before, we express $\pi_{ij}$ as a function of a set of risk characteristics

$$
 P(y_{ij} = 1 | \boldsymbol{x}_{ij}) = \pi(\boldsymbol{x}_{ij}).
$$

In a simple logistic regression model, we have that
$$
 \pi(\boldsymbol{x}_{ij}) = \frac{1}{1 + e^{-\boldsymbol{x}_{ij}^\top \boldsymbol{\beta}}}.
$$

We have a perfectly calibrated model when $P(y_{ij} = 1 | \pi(\boldsymbol{x}_{ij})) = \pi(\boldsymbol{x}_{ij})$ for all $i$ and $j$. To estimate the calibration, we could extend \@ref(eq:logcal) to

$$
  \text{logit}(P({}_{*} y_{ij} = 1| {}_{*} \widehat{\pi}_{ij})) = \alpha + \zeta \ \text{logit}({}_{*} \widehat{\pi}_{ij}).
$$

However, with this approach, we ignore the clustering and do not obtain any information on the performance in the different clusters. Since there is no commonly accepted way to estimate the calibration curves in this scenario, @ClusteredCalibration propose three approaches to obtain calibration curves:

 1. [Clustered Group Calibration (CG-C)](#CGC): a bivariate random effects meta-analysis model;
 2. [Two-stage Neta-Analysis Calibration (2MA-C)](#2MAC): a two-stage univariate random effect meta-analytical approach;
 3. [Mixed model Calibration (MIX-C)](#MIXC): a one-step approach using a random-effects model.


All three approaches are implemented in this `CalibrationCurves` package and you can access them through the `valProbCluster` function. To illustrate the methods, we use a clustered training and validation data set.
...
## Clustered Group Calibration (CG-C) {#CGC}
The CG-C method is an extension of the traditional grouped calibration method (binning calibration) to data sets with a clustered structure. In the traditional grouped calibration:

 - all data is pooled;
 - observations are divided into equal-sized groups based on the predicted risks;
...
Conversely, in CG-C:
 ...
 - cluster-level results are combined across clusters via a bivariate random-effects meta-analysis, capturing between-cluster heterogeneity and within-cluster sampling error.

In the package, two grouping strategies are available:
 
 - `"grouped"`: equal-sized quantile groups within each cluster;
 - `"interval"`: fixed-width intervals in (0, 1) within each cluster.
 
### Mathematical details
The CG-C method proceeds as follows:
 
 1. Per cluster $j$, we split the predicted probabilities into $Q$ groups;
 2. Within each group $q$ and for each cluster $j$, we compute the mean observed outcome ($\hat{y}_{qj}$) and mean predicted probability ($\hat{\bar{\pi}}_{qj}$).
 3. Per group $q$, we perform a bivariate random-effects meta-analysis. We include $\text{logit}(\hat{y}_{qj})$ and $\text{logit}(\hat{\bar{\pi}}_{qj})$ as the response variables and we capture the cluster-specific deviation by including a random intercept. When fitting the model, we use an unstructured variance–covariance matrix.
 
 
The equation of this model is given by 
$$
\begin{bmatrix}
\text{logit}(\bar{y}_{qj}) \\
\text{logit}(\hat{\bar{\pi}}_{qj})
\end{bmatrix}
=
\begin{bmatrix}
{}_{\bar{y}} \mu_q + u_{qj} + \varepsilon_{qj} \\
{}_{\hat{\bar{\pi}}} \mu_q + \nu_{qj} + \epsilon_{qj}
\end{bmatrix}
\tag{2}
$$

Where $\bar{y}_{qj}$ denotes the prevalence of cluster $j$ within quantile $q$, and $\hat{\bar{\pi}}_{qj}$ the average estimated risk in cluster $j$ within quantile $q$. We use the subscript $q$ in the quantities to indicate that this refers to quantile $q$. $u_{qj}$ and $\nu_{qj}$ represent the random intercepts for cluster $j$, capturing the between-cluster heterogeneity, whereas $\varepsilon_{qj}$ and $\epsilon_{qj}$ account for the within-cluster error. In this model, we assume that ${}_{\bar{y}} \mu_q + u_{qj}$ and ${}_{\hat{\bar{\pi}}} \mu_q + \nu_{qj}$ are randomly drawn from a distribution with mean (or pooled) ${}_{\bar{y}} \mu_q$ and ${}_{\hat{\bar{\pi}}} \mu_q$, respectively.
...
\Omega_q =
\begin{bmatrix}
{}_{\bar{y}} \tau_q^2 & {}_{b} \rho_q \ \ \ {}_{\bar{y}} \tau_q^2 \ \ {}_{\bar{\hat{\pi}}} \tau_q^2 \\
{}_{b} \rho_q \ \ \ {}_{\bar{y}} \tau_q^2 \ \ {}_{\bar{\hat{\pi}}} \tau_q^2 & {}_{\bar{\hat{\pi}}} \tau_q^2
\end{bmatrix}
$$
...
\Sigma_{qj} =
\begin{bmatrix}
{}_{\bar{y}} \sigma_{qj}^2 & {}_{w} \rho_q \ \ {}_{\bar{y}} \sigma_{qj}^2 \ \ {}_{\bar{\hat{\pi}}} \sigma_{qj}^2 \\
{}_{w} \rho_q \ \ {}_{\bar{y}} \sigma_{qj}^2 \ \ {}_{\bar{\hat{\pi}}} \sigma_{qj}^2 & {}_{\bar{\hat{\pi}}} \sigma_{qj}^2
\end{bmatrix}
$$

We use ${}_{\bar{y}} \sigma_{qj}^2$ and ${}_{\bar{\hat{\pi}}} \sigma_{qj}^2$ to denote the variance of $\bar{y}_{qj}$ and $\hat{\bar{\pi}}_{qj}$, respectively. ${}_{w} \rho_q$ represents the within-cluster correlation and ${}_{b} \rho_q$ the between-cluster correlation in quantile $q$. The between-study variance is denoted as ${}_{\bar{\hat{\pi}}} \tau_q^2$ and ${}_{\bar{\hat{\pi}}} \tau_q^2$.

For each quantile $q$, we use a bivariate meta-analysis model to account for the strong dependence between the average estimated risk and observed proportion. The calibration plot is then created by plotting the $Q$ values of ${}_{\bar{\hat{\pi}}} \mu_q$ on the x-axis and ${}_{\bar{y}} \mu_q$ on the y-axis.

### Illustration

The following code gives an example of the CG-C (Grouped).
```{r CGC}
res_cgcg <- valProbCluster(
  data = valClusterData,
  p = preds, y = y, cluster = center,
  approach = "CGC", method = "grouped"
)
res_cgcg
```

You can employ the CG-C (Interval) using the following code. This follows the same steps as the Grouped method, except for Step 1 where it uses equally spaced probability intervals instead of quantiles.

```{r CGCinterval}
res_cgci <- valProbCluster(
  data = valClusterData,
  p = preds, y = y, cluster = center,
  approach = "CGC", method = "interval"
)
res_cgci
```


We interpret the plot as follows:
...
## Two stage meta-analysis calibration (2MA-C) {#2MAC}
For the second approach, we use a two-stage random effects meta-analysis to estimate the calibration plot. First, we fit a flexible curve per cluster with a smoother of choice. Currently, the user can choose between restricted cubic splines and LOESS. When employing restricted cubic splines, the number of knots per cluster is selected by performing a likelihood ratio test between all combinations of models with 3, 4, and 5 knots, selecting the model with the least knots that provides the best fit. When you opt for LOESS, the span parameter with the lowest bias-corrected AIC is selected in each cluster. 

### Mathematical details
We train a flexible curve per cluster and estimate the observed proportion for a fixed grid of estimated risks from 0.01 to 0.99 ($G$, default is 100 points). Hereafter, we use a random effects meta-analysis model per point in the grid to combine the logit-transformed predictions across clusters, fitting the following univariate model per point $g$ in the grid:

$$
\text{logit}({}_{s} \hat{\pi}_{gj}) = {}_{\hat{\pi}} \mu_g + \nu_{gj} + \epsilon_{gj}
$$

${}_{s} \hat{\pi}_{gj}$ denotes the predicted proportion of point $g$ for cluster $j$, ${}_{\hat{\pi}} \mu_g$ the overall mean for point $g$, $\nu_{gj}$ the cluster-specific deviation, and $\epsilon_{gj}$ the error.

We assume that $\nu_{gj} \sim \mathcal{N}(0, {}_{\hat{\pi}} \tau_g^2)$ and $\epsilon_{gj} \sim \mathcal{N}(0, {}_{\hat{\pi}} \sigma_{gj}^2)$. Further, we include the inverse of the variance of $\text{logit}({}_{s} \hat{\pi}_{gj})$ as weight and hereby take both the cluster-specific (${}_{\hat{\pi}} \sigma_{gj}^2$) and between-cluster variability (${}_{\hat{\pi}} \tau_g^2$) into account.
...
<!-- Detailed methodological overview can be found in the published [article](https://doi.org/10.1017/rsm.2025.10046). -->



### Illustration
The code below shows how to estimate the calibration curves using the MAC2 method, using splines and LOESS in the first step.

```{r MAC2, warning = FALSE}
res_mac2_spl <- valProbCluster(
  data = valClusterData,
  p = preds, y = y, cluster = center,
  approach = "MAC2", knots = 3,
...
$$
\text{logit}\left(P(y_{ij} = 1 \mid \widehat{\pi}(\boldsymbol{x}_{ij}), \widetilde{s}_j)\right) = s\left(\text{logit}\left(\widehat{\pi}(\boldsymbol{x}_{ij})\right)\right) + \tilde{s}_j \left(\text{logit}\left(\widehat{\pi}(\boldsymbol{x}_{ij})\right)\right).
$$

Here, $s$ denotes a smooth effect and $\tilde{s}_j$ denotes the smooth random effect for cluster $j$. The model is fit using the \code{lme4} package, and splines are added with the \code{rms} package. This approach estimates the calibration per cluster and the variance of the random effects in a single step.
...
The prediction intervals are calculated using the `predictInterval` function in **R** with 10,000 samples (simulation-based). This function takes the following into account:
...
### Illustration
Using the code as shown below, we can estimate the calibration curve using the MIX-C method.
```{r}
res_mixc_slo <- valProbCluster(
  p = valClusterData$preds, y = valClusterData$y, cluster = valClusterData$center,
  pl = TRUE,
  approach = "MIXC", method = "slope"
)
res_mixc_slo
```
...
## Default approach {#default}
The default approach in `valProbCluster` combines the strengths of 2MA-C and MIX-C. It uses 2MA-C with splines for the overall (pooled) calibration curve, confidence intervals, and prediction intervals, while relying on MIX-C for the cluster-specific curves. This leverages the robust pooled calibration estimates from 2MA-C and the shrinkage properties of MIX-C for the cluster-specific curves, which is particularly beneficial when sample sizes per cluster are small.

This approach is used when `approach = "default"` is specified, or simply when the `approach` argument is omitted altogether.

### Illustration
```{r default, warning = FALSE}
res_default <- valProbCluster(
  p = valClusterData$preds, y = valClusterData$y, cluster = valClusterData$center
)
res_default
```

To display the cluster-specific curves, set `cluster_curves = TRUE`.
```{r default_cc, warning = FALSE}
...
- **Black (dashed)**: Pooled calibration curve across clusters (estimated via 2MA-C with splines)
- **Black (solid)**: Cluster-specific curves (estimated via MIX-C, shown when `cluster_curves = TRUE`)
- **Shaded areas**:
  - Dark gold = 95% Confidence Interval (uncertainty around the pooled curve)
  - Light gold = 95% Prediction Interval (expected calibration curve in new clusters)
...
# Competing-risks calibration

When the outcome involves multiple competing events (e.g. disease-specific death vs. other-cause death), standard survival calibration is insufficient.  The `valProbCompRisks()` function assesses calibration for competing-risks prediction models fitted with `CSC()` (cause-specific hazards) or `FGR()` (Fine-Gray subdistribution hazards) from the **riskRegression** package.

The methodology closely follows @CompRisks2022.  Briefly:

1. Predicted risks at the specified time horizon are obtained via `predictRisk()`.
2. An O/E ratio is computed using the Aalen-Johansen estimator as the "observed" cumulative incidence.
3. A calibration curve is estimated via a Fine-Gray model with restricted cubic splines (RCS) on the complementary log-log scale.
4. When `pseudo = TRUE`, pseudo-observations (from `riskRegression::Score`) are smoothed with loess and used for calibration intercept/slope estimation via GEE.

```{r comprisks-example, eval = FALSE}
library(riskRegression)
library(survival)
# (Assumes cscFit is a CSC model and testCompData is the validation data)
calComp <- valProbCompRisks(
  cscFit,
  valdata     = testCompData,
  cause       = 1,
  timeHorizon = 5,
  plotCal     = "ggplot"
)
...
# Ordinal and multinomial calibration

For prediction models with ordered or nominal categorical outcomes, `valProbMulticat()` assesses calibration following Van Hoorde et al. (2014, 2015) and @VanCalster2019.

- **Ordinal** (`polr`): per-category calibration curves on cumulative probabilities.
- **Multinomial** (`multinom`): one-vs-rest calibration curves per category.

Both approaches return proper scoring rules (Brier score, log-loss) and per-category statistics (ICI, E50, E90, Emax).

```{r multicat-example, eval = FALSE}
library(MASS)
library(nnet)
...
oFit <- polr(type ~ ., data = Pima.tr, Hess = TRUE)
calOrd <- valProbMulticat(oFit, Pima.te, plotCal = "ggplot")
calOrd

# Multinomial example
mFit <- multinom(Species ~ ., data = iris[1:100, ], trace = FALSE)
calMn <- valProbMulticat(mFit, iris[101:150, ], plotCal = "ggplot")
calMn
```

# Recalibration

Even a well-discriminating model may be miscalibrated when applied to a new population.  The `recalibrate()` function provides two post-hoc recalibration methods:

- **Platt scaling**: fits a logistic regression of the outcome on the logit of the predicted probabilities, yielding a linear recalibration of the log-odds scale (@Platt1999).
- **Isotonic regression**: fits a monotone step-function mapping using the pool-adjacent-violators algorithm (@FawcettNiculescu2007).

The function returns recalibrated predictions together with before/after comparisons of Brier score, log-loss, calibration slope and intercept.  It also produces two diagnostic plots: a before/after calibration curve comparison and a transformation plot.

```{r recalibrate-example, eval = FALSE}
# Using the binary logistic example from above
pHat <- predict(glmFit, newdata = testdata, type = "response")
yTest <- testdata$y

# Platt scaling
recalPlatt <- recalibrate(pHat, yTest, method = "platt")
recalPlatt

# Isotonic regression
recalIso <- recalibrate(pHat, yTest, method = "isotonic")
recalIso
```

# Unified `calibrate()` interface

To simplify the workflow, the `calibrate()` function automatically detects the model class and dispatches to the appropriate calibration function:

| Model class | Dispatches to |
|-------------|---------------|
| `glm` (binomial) | `valProbggplot` |
| `glm` (other family) | `genCalCurve` |
| `coxph` | `valProbSurvival` |
| `CSC` / `FGR` | `valProbCompRisks` |
| `polr` / `multinom` | `valProbMulticat` |

```{r calibrate-example, eval = FALSE}
# Binary logistic model — automatically calls valProbggplot
calibrate(glmFit, newdata = testdata, y = "y")

# Cox model — automatically calls valProbSurvival
