MAC2_old <- function(data = NULL, preds, y, cluster, grid_length = 100, methods = c("log", "loess", "splines", "kde"),
                     ...
                     hakn = FALSE, linewidth = 1, method.predict = "HTS") {
  ...
  #' @param cluster_curves Logical, indicating whether to cluster curves at the mean. Default is FALSE.
  ...
  data <- data.frame(predictions = preds, cluster = cluster, outcome = y)
  grid <- seq(0.01, 0.99, length.out = grid_length)
  data_all_lp <- data.frame()

  transform_function <- if (transf == "logit") qlogis else identity
  for (subcluster in unique(cluster)) {
    risk_cluster <- data %>% filter(cluster == subcluster)
    observed_grid <- data.frame(x = grid, cluster = subcluster, nsample = nrow(risk_cluster))
    ...
    if ("splines" %in% methods) {
      ...
      splines_data <- data.frame(splines = splines_data$linear.predictors, splines_se = splines_data$se.fit, knots_used = knots_sub)
      observed_grid <- cbind(observed_grid, splines_data)

      message("Spline model fitted with ", knots_sub, " knots.")
    }
  ...
  if (cluster_curves) {
    p <- p + geom_line(data = data_all_lp, aes(x = x, y = plogis(data_all_lp[, method_choice]), group = cluster), lwd = linewidth / 2, show.legend = F, lty = "solid")
  }
  if (plot) print(p)
